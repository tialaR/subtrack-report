...
export const usePatchGeneralMapScreenshotMarkers = () => {
  ...
  const patch = async (id: string, payload: Partial<GeneralMapScreenshotMarker>) => {
    setLoading(true);
    try {
      const response = await api.patch<GeneralMapScreenshotMarker>(`/general_map_screenshot_markers/'${id}`, payload);
